(function ($) {
    ...
    this.Weavy = function () {
        ...
        var defaults = {
            ...
            className: 'weavy-middle weavy-collapsed',
            add_title: 'Open',
            snappingY: 4,
            snappingX: 4,
        ...
        Weavy.prototype.collapse = function () {
            if (!$(self.container).is(".weavy-no-collapse, .weavy-open, .weavy-before-drag, .weavy-drag, .weavy-snap")) {
                $(self.container).addClass("weavy-collapsed");
                minimizeTimeout.apply(self);
                triggerEvent("collapse", null);
            }
        }
        ...
        Weavy.prototype.minimize = function () {
            if (!$(self.container).is(".weavy-no-collapse, .weavy-open, .weavy-before-drag, .weavy-drag, .weavy-snap")) {
                var isCollapsed = $(this.container).hasClass("weavy-collapsed");
                $(self.container).addClass("weavy-collapsed weavy-minimized");
                if (!isCollapsed) {
                    triggerEvent("collapse", null);
                }
                triggerEvent("minimize", null);
            }
            clearMinimizeTimeout.apply(self);
        ...
        Weavy.prototype.restore = function () {
            $(self.container).removeClass("weavy-collapsed weavy-minimized");
            triggerEvent("restore", null);
            clearMinimizeTimeout.apply(self);
            setTimeout(preloadFrames, 2000, "all"); // Grace time until preloading
        }
        ...
        function minimizeTimeout() {
            ...
            if ($(this.container).hasClass("weavy-collapsed")) {
                minimizeTimer = setTimeout(this.minimize.bind(this), this.options.minimize_delay || 30000);
            }
        }
        ...
        function openContextFrame(weavyContext) {
            if (weavyContext) {
        ...
        function preloadFrames(force) {
            ...
            if (!preloading || force) {
                ...
                if ($systemFrames.length) {
                    ...
                    if ($strips.length) {
                        preloadFrame($strips[0]);
                        setTimeout(preloadFrames, 1500, "all");
                    }
                }
            }
        }
        ...
        function buildOutput() {
            ...
            this.container.classList.remove("weavy-before-drag");
            ...
            if (this.options.user_id) {
                ...
                this.container.classList.remove("weavy-collapsed", "weavy-minimized");
                this.container.classList.add("weavy-no-collapse");
            }

            // version mismatch
            if (self.options.should_update && chrome && chrome.runtime) {
                try {
                    chrome.runtime.sendMessage(null, { name: "sync" }, function (response) {
                        // show message
                        showAlert.call(this, "<strong>Weavy has been upgraded</strong><br/>Reload page to get the latest version.", true);
                    });
                } catch (e) { }
            }

            minimizeTimeout.apply(this);
        }
        ...
        function cleanBubblesList(bubblesList) {
            ...
            bubblesList.forEach(function (bubble) {
                ...
                bubble.url = httpsUrl(bubble.url);
        ...
        function addAndRemoveBubbles(newBubbles) {
            if (newBubbles) {
                ...
                _.each(newBubbles, function (newBubble) {
                    newBubble.url = httpsUrl(newBubble.url);
        ...
        function extendDefaults(source, properties) {
            ...
            for (property in properties) {
                if (properties.hasOwnProperty(property)) {
                    source[property] = httpsUrl(properties[property], https);
                }
            }
            return source;
        }

        function httpsUrl(url, https) {
        ...
        function initializeEvents() {
            ...
            this.draggable.addEventListener("mousedown", dragBeforeStart, true);
            this.draggable.addEventListener("mouseup", dragBeforeStartEnd, false);
            this.draggable.addEventListener("dragstart", dragStart, false);
            this.draggable.addEventListener("dragend", drop, false);
            this.draggable.addEventListener("click", this.restore.bind(this));
            this.draggable.addEventListener("mouseover", minimizeTimeout.bind(this), { passive: true });

            document.documentElement.addEventListener("mousedown", this.collapse.bind(this), false);
        }
        ...
        function setDragPosition(x, y, forceEdge) {
            ...
            var dw = $(self.draggable).outerWidth()
            var mt = parseInt($(self.draggable).css("margin-top")) || 0;
            var mb = parseInt($(self.draggable).css("margin-bottom")) || 0;
            var ml = parseInt($(self.draggable).css("margin-left")) || 0;
            var mr = parseInt($(self.draggable).css("margin-right")) || 0;
            ...
            $(self.draggable).css("transform", "translate(" + adjustedX + "px, " + adjustedY + "px)" + ($(self.container).hasClass("weavy-collapsed") ? " scale(.8)" : ""));
        }
        ...
        function getDragPos(ev) {
            ...
            var offsetY = 0;
            if (q === 1 || q === 2) {
                offsetY = ev.clientY - self.dragData.cy - self.dragData.h / scale / 2 - parseInt($draggable.css("margin-top"));
            } else {
                offsetY = h - (ev.clientY - self.dragData.cy + self.dragData.h / scale / 2 + parseInt($draggable.css("margin-bottom")));
            }
        ...
        function onMessageReceived(e) {
            ...
            switch (e.data.name) {
                ...
                case "pong":
                    ...
                    openContextFrame(e.data.context);
                    break;
                case "invoke":
        ...
        function loadInTarget(target, url, data, method, fill) {
            ...
            if (frameTarget) {
                if (frameTarget.dataset && frameTarget.dataset.src) {
                    ...
                    sendToFrame(frameTarget.name, httpsUrl(url), data, method);
                } else {
                    // Fully loaded, send using turbolinks
                    frameTarget.contentWindow.postMessage({ name: 'send', url: httpsUrl(url), data: data, method: method }, "*");
                }
            }
        }
        ...
        weavy.realtime.on("bubbleopened", function (e, data) {
            ...
            if (data.isMessenger) {
                self.messengerFrame.src = httpsUrl(data.url);
                loading.call(self, "weavy-strip-messenger", true);
                self.open("messenger");
        ...
        weavy.realtime.on("loaded", function (e, data) {
            ...
            if (self.options.ext_auth_provider) {
                // make sure frame is loaded
                //preloadFrame(self.personalFrame);
            }
